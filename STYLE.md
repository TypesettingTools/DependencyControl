...
- **NM1.** Casing **MUST** be: `camelCase` for methods, fields, and locals; `PascalCase` for classes and `Enum` members; `UPPER_SNAKE` for module-level constants. `snake_case` appears only at the Aegisub API and C FFI boundaries, never in project-native code. A compound camelCase name **MUST** show its word boundaries rather than run the words together — `feedLoader`, not `feedloader`. Each segment **SHOULD** be a whole word, not a truncation (`digestLen` over `dlen`), though short conventional abbreviations like `id`, `ns`, `len`, and `msg` are fine. An acronym or initialism **MUST** be cased as an ordinary word — `feedUrl`, `toJson`, `setConsoleOutputUtf8`, never `feedURL` or `toJSON`. A run of capitals loses the boundary where two of them meet (`parseHTTPURL`), and it has no single kebab-case form for a file name to mirror ([MOD1](#mod--files--modules)), since splitting on case gives `u-t-f-8` where splitting on runs gives `utf8`. A name that mirrors an external source — a C symbol, a grammar node type, another project's published API — keeps that source's spelling, so the `SHA1` in the `DownloadManager` shim and ICU's `u_strFromUTF8WithSub` stay as they are.
...
- **NM5.** A function returning an iterator for `for … in` **MUST** take the `iterate` prefix (`iterateChars`, `iterateSegments`, `iteratePackages`). A bare noun costs more here than [NM3](#nm--naming) charges it elsewhere: the return value is usable in exactly one syntactic position, Lua has no iterator type to catch a misuse, and a `---@return fun(): …` is easy to skim past, so the name is the only signal at the call site. `walk` **MUST NOT** be used for this — it is reserved for a traversal that does the work itself and returns a result or nothing, as MoonCats' `walkClass` does, so the two kinds never have to be told apart by reading the body. A stand-in for an external API keeps that API's spelling instead ([NM1](#nm--naming)), which is why the `aegisub.re` and `aegisub.unicode` shims keep `gsplit`, `gfind` and `chars`.
...
- **PV4.** A mock-worthy dependency — an injected collaborator that is a test seam (TS5) — **MUST** live in a `---@private` class field (`@@__dep`/`@__dep`) that a test can swap, not a module-local upvalue nothing outside the file can replace.
...
- **TS5.** A member is a *test seam* when a test must intercept what it does rather than let it run — I/O, filesystem or network access, global or config mutation, or non-determinism (time, randomness). A seam **MUST** be reachable for stubbing — an injectable `---@private` field or a `---@private __method` reachable via `.__base`, not a class-body-local or module-local function nothing outside the file can replace. A pure, deterministic helper is not a seam and stays a plain function.
