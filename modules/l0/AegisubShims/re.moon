...
unsupportedFlags = {NEWLINE_ALT: true}
...
combineFlags = (flags, errorLevel) ->
  combined = 0
  for flag in *flags
    error msgs.combineFlags.unsupported\format(flag.name), errorLevel + 1 if unsupportedFlags[flag.name]
...
toPcreOptions = (flags) ->
  ...
  options = baseOptions
  options = bit.bor options, pcre.CASELESS if hasFlag flagValues.ICASE
  options = bit.bor options, pcre.EXTENDED if hasFlag flagValues.MOD_X
  options = bit.bor options, pcre.NO_AUTO_CAPTURE if hasFlag flagValues.NOSUB
  -- MOD_S only restates the default; its negation is the one that changes anything
  options = bit.band options, bit.bnot pcre.DOTALL if hasFlag flagValues.NO_MOD_S
  options = bit.band options, bit.bnot pcre.MULTILINE if hasFlag flagValues.NO_MOD_M
  return options
...
Re[name] = setmetatable {:name, :value}, flagMeta for name, value in pairs flagValues
