...
{:FontWeight} = require "l0.AssParser.ass"
{:gdi32, :LogFontW, :Size, :TextMetricW, :MapMode, :OutputPrecision, :ClipPrecision,
  :FontQuality, :FontPitch, :FontFamily, :CharSet} = ffiGdi
-- eager-load all used GDI symbols so we error out early (and on non-Windows test runs) when an undeclared one is used
{:CreateCompatibleDC, :CreateFontIndirectW, :DeleteDC, :DeleteObject, :GetTextExtentPoint32W,
  :GetTextMetricsW, :SelectObject, :SetMapMode} = gdi32
...
measureSpaced = (deviceContext, wide, unitCount, spacing) ->
  ...
  for offset = 0, unitCount - 1
    return nil if 0 == GetTextExtentPoint32W deviceContext, wide + offset, 1, extent
...
measure = (style, text) ->
  ...
  SetMapMode deviceContext, MapMode.Text
  ...
  if spacing != 0
    ...
    extent = Size!
    measured = GetTextExtentPoint32W deviceContext, wideText, unitCount, extent
    fail msgs.measure.noExtent\format(text, ffiWindows.describeLastError!) if measured == 0
    width, height = extent.cx, extent.cy

  metrics = TextMetricW!
  reported = GetTextMetricsW deviceContext, metrics
...
---Measures text through GDI, the way Aegisub does on Windows, matching it exactly, quirks and all.
