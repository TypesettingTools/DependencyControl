-- Declarations are transcribed from Pango 1.58.1's pango-layout.h, pango-font.h, pango-fontmap.h and
-- pangocairo.h, plus GObject's gobject.h for the one release call.
--
...
---Pango's calls for laying out and measuring text, with the pangocairo entry point that provides a
---font map without a display.
---@class FfiPango
...
return {
  ...
  -- exposed even where the libraries did not load, so a module can bind the names it uses at load
  -- time; each then stands in for its library and throws when called
  ---@type ffi.namespace*
  pango: pangoBinding.functions

  ---@type ffi.namespace*
  pangocairo: pangocairoBinding.functions

  ---@type ffi.namespace*
  gobject: gobjectBinding.functions
