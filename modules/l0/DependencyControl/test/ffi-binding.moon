...
->
...

  {
    ...
    -- a data symbol has no parameter list, so the function pass cannot match it
    rewrite_aliasesADataSymbolToItsRealSymbol: (ut) ->
      rewritten = rewrite "extern const void* kThingAttributeName;", {}, {}, {"kThingAttributeName"}
      ut\assertNotNil rewritten\find(
        "#{DEPCTRL_SHORT_NAME}kThingAttributeName __asm__(\"kThingAttributeName\");", 1, true)

    -- a symbol whose address is all a caller wants is declared as an array, whose bounds have to stay
    -- between the declarator and the label
    rewrite_aliasesADataSymbolDeclaredAsAnArray: (ut) ->
      rewritten = rewrite "extern const uint8_t kThingCallBacks[64];", {}, {}, {"kThingCallBacks"}
      ut\assertNotNil rewritten\find(
        "#{DEPCTRL_SHORT_NAME}kThingCallBacks[64] __asm__(\"kThingCallBacks\");", 1, true)

    rewrite_reportsAListedVariableTheDeclarationsLack: (ut) ->
      ut\assertErrorMsgMatches (-> rewrite "int unrelated;", {}, {}, {"kMissing"}), {}, "kMissing"

    -- a struct named in the symbol's own declaration still gets prefixed after it
    rewrite_prefixesATypeNamedInADataSymbolsDeclaration: (ut) ->
      rewritten = rewrite "typedef struct Thing Thing; extern Thing* kThing;", {"Thing"}, {}, {"kThing"}
      ut\assertNotNil rewritten\find(
        "#{DEPCTRL_SHORT_NAME}Thing* #{DEPCTRL_SHORT_NAME}kThing __asm__(\"kThing\");", 1, true)
    ...
    bind_reportsAMissingLibraryUnavailable: (ut) ->
      ...
      -- the table stands in for the library so a module can still bind its names at load time, and
      -- stays empty so that rawget remains the way to ask whether a symbol resolved
      ut\assertNil rawget binding.functions, "FfiBindingTestNothing"
      ut\assertFunction binding.functions.FfiBindingTestNothing
      ut\assertErrorMsgMatches (-> binding.functions.FfiBindingTestNothing!), {},
        "did not load"

    bind_aMissingLibraryStillRejectsAnUndeclaredName: (ut) ->
      binding = ffiBinding.bind {
        declarations: "int FfiBindingTestNothing(void);"
        functions: {"FfiBindingTestNothing"}
        library: {"no-such-library-anywhere"}
      }
      ut\assertErrorMsgMatches (-> binding.functions.FfiBindingTestNotDeclared), {}, "not declared"
      ut\assertFalse binding.hasSymbol "FfiBindingTestNothing"

    -- a library can load and still lack a symbol, which is the case hasSymbol answers and
    -- isAvailable cannot
    hasSymbol_separatesAResolvedSymbolFromALoadedLibrary: (ut) ->
      binding = ffiBinding.bind {
        namespace: ffi.C
        declarations: "int FfiBindingTestAbsent(void);"
        functions: {"FfiBindingTestAbsent"}
      }
      ut\assertTrue binding.isAvailable
      ut\assertFalse binding.hasSymbol "FfiBindingTestAbsent"
      ut\assertFalse binding.hasSymbol "FfiBindingTestNeverDeclared"
