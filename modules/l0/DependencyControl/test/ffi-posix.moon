...
() ->
...

  {
    ...
    create_makesFile: (ut) ->
      path = tmpPath!
      fd = ffiPosix.open path, OpenFlags\combine(OpenFlags.Write, Create), ffiPosix.getFileMode "rw"
    ...
    exclusive_failsOnExisting: (ut) ->
      path = tmpPath!
      fd1 = ffiPosix.open path, OpenFlags\combine(OpenFlags.Write, Create), ffiPosix.getFileMode "rw"
      ffiPosix.close fd1 if fd1 >= 0
      fd2 = ffiPosix.open path, OpenFlags\combine(OpenFlags.Write, Create, Exclusive), ffiPosix.getFileMode "rw"
    ...
    getFileMode_setsPermissions: (ut) ->
      ...
      path = tmpPath!
      fd = ffiPosix.open path, OpenFlags\combine(OpenFlags.Write, Create), ffiPosix.getFileMode "rw", "r", "r"
      ffiPosix.close fd if fd >= 0
      ffi.C.umask oldMask
