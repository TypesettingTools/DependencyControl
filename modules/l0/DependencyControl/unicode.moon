...
leadMarkerByWidth = {0, LEAD_MARKER_TWO_BYTE, LEAD_MARKER_THREE_BYTE, LEAD_MARKER_FOUR_BYTE}
leadPayloadBitsPerWidth = {7, 5, 4, 3}
LEAD_PAYLOAD_MASK_PER_WIDTH = [2^bits - 1 for bits in *leadPayloadBitsPerWidth]
...
getLeadByteFor = (codePoint, width) ->
  bor leadMarkerByWidth[width], rshift codePoint, (width - 1) * CONTINUATION_PAYLOAD_BITS
...
for leadByte = SMALLEST_TWO_BYTE_LEAD, LARGEST_LEAD
  ...
  lowestCodepoint = math.max lowestCodepoint, shortestCodePointByWidth[width]
...
decodeSequenceAegisub = (text, byteOffset) ->
  ...
  codePoint = leadByte - leadMarkerByWidth[width]
  for offset = 1, width - 1
...
Unicode = {
  ...
  encodeUtf8: (codePoints, mode = DecodeMode.Strict) ->
    ...
    for index, codePoint in ipairs codePoints
      ...
      width = 1
      while width < #shortestCodePointByWidth and
        codePoint >= shortestCodePointByWidth[width + 1]
        width += 1
      ...
      sequence[1] = bor leadMarkerByWidth[width], remaining
